...
func (s *Server) registerLegacyRoutes(mux *http.ServeMux) {
    ...
    mux.HandleFunc("POST /api/qr/verify", s.qrVerify)
    ...
    mux.HandleFunc("/api/media/", s.notMigrated)
...
func (s *Server) qrVerify(w http.ResponseWriter,_ *http.Request){writeJSON(w,http.StatusNotImplemented,map[string]string{"error":"二维码业务尚未迁移到 go-api"})}
