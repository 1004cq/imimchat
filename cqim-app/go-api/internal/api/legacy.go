...
func (s *Server) registerLegacyRoutes(mux *http.ServeMux) {
    ...
    mux.HandleFunc("POST /api/auth/reset-password", s.authResetPassword)
    mux.HandleFunc("POST /api/auth/bind-phone", s.requireUser(s.authBindPhone))
    mux.HandleFunc("POST /api/auth/bind-email", s.requireUser(s.authBindEmail))
...
func (s *Server) authLogin(w http.ResponseWriter, r *http.Request) {
	...
	if decode(r, &in) != nil || in.Account == "" || in.Password == "" {
		writeJSON(w, 400, map[string]string{"error": "请输入账号和密码"})
		return
	}
	var id, username, hash, nickname, avatar, bio string
	var dialogID, phone, email *string
	var isBot, phoneVerified, emailVerified, banned bool
	e := s.db.QueryRow(r.Context(), `SELECT "id","dialogId","username","password",COALESCE("nickname",''),"phone","email",COALESCE("avatar",''),COALESCE("bio",''),"isBot","phoneVerified","emailVerified","isBanned" FROM "User" WHERE "username"=$1 OR "email"=$1 OR "phone"=$1 OR "id"=$1 LIMIT 1`, in.Account).
		Scan(&id, &dialogID, &username, &hash, &nickname, &phone, &email, &avatar, &bio, &isBot, &phoneVerified, &emailVerified, &banned)
	if e != nil || bcrypt.CompareHashAndPassword([]byte(hash), []byte(in.Password)) != nil {
		writeJSON(w, 401, map[string]string{"error": "账号或密码错误"})
		return
	}
	if banned {
		writeJSON(w, 403, map[string]string{"error": "账号已被封禁"})
		return
	}
	token := newID() + newID()
	_, e = s.db.Exec(r.Context(), `INSERT INTO "UserSession" ("id","userId","token","expiresAt","createdAt") VALUES ($1,$2,$3,$4,NOW())`, newID(), id, token, time.Now().Add(30*24*time.Hour))
	if e != nil {
		dbError(w, e)
		return
	}
	...
	writeJSON(w, 200, map[string]any{
		...
		"token":   token,
		"user": map[string]any{
			"id": id, "dialogId": dialogID, "username": username, "nickname": nick,
			"phone": phone, "email": email, "avatar": safeAvatarUrl(avatar), "bio": bio,
			"isBot": isBot, "phoneVerified": phoneVerified, "emailVerified": emailVerified,
		},
	})
}
func (s *Server) authRegister(w http.ResponseWriter, r *http.Request) {
	...
	if decode(r, &in) != nil || in.Username == "" || in.Password == "" {
		writeJSON(w, 400, map[string]string{"error": "请填写用户ID和密码"})
		return
	}
	if len(in.Username) > 20 {
		writeJSON(w, 400, map[string]string{"error": "用户ID格式不正确"})
		return
	}
	...
	_, e = s.db.Exec(r.Context(), `INSERT INTO "User" ("id","username","password","nickname","createdAt","updatedAt") VALUES ($1,$2,$3,$4,NOW(),NOW())`, id, in.Username, string(hash), nick)
	if e != nil {
		writeJSON(w, 409, map[string]string{"error": "该用户ID已被使用"})
		return
	}
	_, e = s.db.Exec(r.Context(), `INSERT INTO "UserSession" ("id","userId","token","expiresAt","createdAt") VALUES ($1,$2,$3,$4,NOW())`, newID(), id, token, time.Now().Add(30*24*time.Hour))
	if e != nil {
	...
	writeJSON(w, 200, map[string]any{
		...
		"token":   token,
		"user": map[string]any{
			"id": id, "dialogId": nil, "username": in.Username, "nickname": nick,
			"phone": nil, "email": nil, "avatar": "", "bio": "",
			"isBot": false, "phoneVerified": false, "emailVerified": false,
		},
	})
}
