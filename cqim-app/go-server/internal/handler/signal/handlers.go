	...
	case "call_invite":
		{
			if to != "" {
				...
				if !s.isUserOnline(to) {
					...
					_ = push.SendApnsToUser(s.deps, to, "来电", callerName+" 邀请你"+map[bool]string{true: "视频通话", false: "语音通话"}[callType == "video"],
						map[string]any{"type": "call_invite", "from": c.userID, "callType": callType})
				}
			}
		}
