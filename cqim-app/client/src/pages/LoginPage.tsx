...
import { Lock, Shield, Phone, Mail, ArrowLeft, Eye, EyeOff, KeyRound, Sparkles } from 'lucide-react';
...
export default function LoginPage() {
  ...
  const inputCls = [
    'w-full h-14 px-4 rounded-2xl text-[15px] text-white placeholder:text-white/30',
    'bg-white/[0.055] border border-white/[0.13] shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]',
    'focus:outline-none focus:border-[#4ade80]/70 focus:bg-white/[0.085]',
    'focus:shadow-[0_0_0_4px_rgba(74,222,128,0.10),inset_0_1px_0_rgba(255,255,255,0.04)]',
    'transition-all duration-200',
  ].join(' ');

  // 主按钮：绿色渐变，带光晕
  const btnPrimaryCls = [
    'w-full h-14 rounded-2xl text-[15px] font-semibold text-white',
  ...
  // 验证码按钮
  const btnCodeCls = [
    'px-4 h-14 rounded-2xl text-xs font-semibold whitespace-nowrap',
    'bg-white/[0.06] border border-white/[0.13] text-white/75',
    'hover:bg-white/[0.10] hover:text-white',
    'disabled:opacity-40 transition-all duration-200',
  ].join(' ');

  const labelCls = 'text-xs text-white/45 pl-1 mb-2 block font-medium tracking-wide';
  ...
  return (
    <div className="h-full flex flex-col relative overflow-hidden" style={{ background: '#070909' }}>

      {/* 背景光晕装饰 */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        {/* 顶部绿色光晕 */}
        <div
          className="absolute -top-40 left-1/2 -translate-x-1/2 w-[560px] h-[360px] rounded-full opacity-20"
          style={{ background: 'radial-gradient(ellipse, #22c55e 0%, transparent 70%)', filter: 'blur(60px)' }}
        />
        {/* 底部蓝色光晕 */}
        <div
          className="absolute -bottom-40 -right-20 w-[420px] h-[320px] rounded-full opacity-10"
      ...
      {/* 内容区 */}
      <div className="relative z-10 flex-1 flex flex-col justify-start px-5 sm:px-6 overflow-y-auto py-10 sm:py-14">
        ...
        >
          ...
          <div className="relative mb-5">
            ...
            />
            <div className="relative w-[88px] h-[88px] rounded-[26px] overflow-hidden border border-white/10 shadow-[0_8px_32px_rgba(0,0,0,0.6)]">
              <img src="/imim-logo-formal.jpg" alt="imim" className="w-full h-full object-cover" />
            </div>
          </div>

          {/* 品牌名 */}
          <h1 className="text-[30px] font-bold text-white tracking-[0.34em] pl-[0.34em] mb-2">IMIM</h1>
          <p className="text-[11px] text-white/35 tracking-[0.24em]">安全 · 私密 · 端对端加密</p>

          {/* 加密徽章 */}
          <div className="flex items-center gap-1.5 mt-4 px-3.5 py-2 rounded-full border border-[#22c55e]/25 bg-[#22c55e]/[0.07] shadow-[0_0_24px_rgba(34,197,94,0.08)]">
            <Shield size={13} className="text-[#4ade80]" />
            <span className="text-[11px] text-[#4ade80]/85 font-medium">端对端加密通讯</span>
          </div>
        </motion.div>
        ...
        <AnimatePresence mode="wait">
          {showContent && (
            <motion.div
              ...
              className="max-w-[390px] mx-auto w-full space-y-5"
            >
              ...
              {mode === 'login' && (
                <div className="space-y-5">
                  {/* Tab 切换 */}
                  <div className="flex rounded-2xl p-1 bg-white/[0.045] border border-white/[0.08]">
                    {loginTabs.map(tab => (
                      <button
                        key={tab.id}
                        ...
                        className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-medium transition-all relative ${
                          loginType === tab.id ? 'text-white bg-white/[0.10] shadow-[0_4px_14px_rgba(0,0,0,0.18)]' : 'text-white/35 hover:text-white/65'
                        }`}
                      >
                        <tab.icon size={12} />
                        {tab.label}
                        {loginType === tab.id && (
                          <motion.div
                            ...
                            className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-6 h-0.5 bg-[#4ade80] rounded-full"
                          />
                        )}
                      </button>
                    ))}
                    ...
                    <motion.div className="space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          ...
                          <button type="button" onClick={() => handleSendCode(account, 'login', 'sms', setCountdown)} disabled={countdown > 0 || loading} className={btnCodeCls}>
                  ...
                  {loginType === 'email' && (
                    <motion.div className="space-y-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                      ...
                      <div>
                        ...
                        <div className="flex gap-2">
                          <input className={inputCls + ' flex-1'} value={code} onChange={e => setCode(e.target.value)} placeholder="6 位验证码" maxLength={6} onKeyDown={e => e.key === 'Enter' && handleLogin()} />
                          <button type="button" onClick={() => handleSendCode(account, 'login', 'email', setCountdown)} disabled={countdown > 0 || loading} className={btnCodeCls}>
                            {countdown > 0 ? `${countdown}s` : '获取'}
                          </button>
                        </div>
                      </div>
                  ...
                  {regPhone && (
                    <div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button type="button" onClick={() => handleSendCode(regPhone, 'register', 'sms', setRegCountdown)} disabled={regCountdown > 0 || loading} className={btnCodeCls}>
