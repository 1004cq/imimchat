...
**当前状态：** TangSengDaoDao 后端仍使用内部固定验证码 `TS_SMSCODE`，但对外注册入口已接入 `register-service` 中间件，由中间件完成真实验证码校验、限流、密码强度校验后再调用后端注册接口。
...
**已落地：**
- `register-service` 支持 `mock` 与 `aliyun_dypns` 短信模式
- 验证码 5 分钟有效、最多 3 次错误、防重放
- 手机号/IP 注册与短信频率限制
- Docker Compose 已集成注册服务并暴露本机 `9091` 给 Nginx 代理
...
├── ✅ 移动端 UI 修复（已完成）
├── 🔲 配置数据库定时备份
├── ✅ 替换固定验证码为短信服务（通过 register-service 中间件对外校验）
└── 🔲 添加 Nginx 安全头 + 限流
...
| 编号 | 问题描述 | 严重程度 | 状态 |
|------|----------|----------|------|
| #001 | 固定验证码 `123456` 可被滥用注册 | 高 | 已通过 register-service 对外注册校验缓解 |
| #002 | 无数据库自动备份 | 高 | 待修复 |
